...
| RAI-PRIV-003 | Privacy-safe telemetry | Privacy & Security | Analysis lifecycle events record category/status/model/retry/latency/size bucket/MIME/language; blocked-key sanitisation excludes image bytes, Base64, identifiers, prompts, and clinical responses | `lib/telemetry/server.ts`, `lib/telemetry/analysis-events.ts`, `app/api/analyze-wound/route.ts` | No | `tests/rai/rai-telemetry.test.ts` | Implemented |
