...
| RAI-TRANS-006 | Multi-view coverage and duplicate disclosure | Transparency | Joint coverage/duplicate disclosure; repeated views non-additive; model-assisted, not registration | `lib/ai/prompts/wound-visual-observation.ts`, `lib/ai/analysis/pipeline.ts` | Yes | `tests/rai/rai-safety.test.ts`, `tests/rai/rai-unsupported-inference.test.ts` | Partially Implemented |
...
| RAI-SAFE-013 | Bounded photographic TBSA classification | Reliability & Safety | Deterministic 0-100 clamp + Minor/Major band on model estimate | `lib/clinical/tbsa.ts`, `lib/ai/analysis/pipeline.ts` | Yes | `tests/unit/tbsa.test.ts`, `tests/rai/rai-safety.test.ts` | Implemented |
