...
| RAI-SAFE-006 | Indication- and weight-gated Parkland | Reliability & Safety | Explicit adult >=15% / child >=10% indication after clinician category selection; deterministic volumes only with supplied weight | `lib/clinical/parkland.ts`, `lib/ai/analysis/pipeline.ts` | Yes | `tests/unit/parkland.test.ts`, `tests/unit/analysis-pipeline.test.ts`, `tests/rai/rai-safety.test.ts` | Implemented |
...
| RAI-SAFE-010 | Safe-failure state | Reliability & Safety | Labelled unavailable result or actionable input rejection; disclaimer preserved | `lib/ai/validation/wound-analysis-schema.ts`, Original HCP analysis client | Yes | `tests/unit/ai-parsing.test.ts`, `tests/api/routes.spec.ts` | Implemented |
...
| RAI-REL-002 | Recoverable analysis failure and API reliability evidence | Reliability & Safety | Retains selected image/form context after failure; exact bilingual retry/replacement actions; repeated safe-demo-image API reliability summary with a >=95% demo target | Original HCP analysis client, `tests/reliability/image-analysis-reliability.ts` | Yes | `tests/e2e/hcp-analysis-retry.spec.ts`, `tests/unit/analysis-reliability.test.ts` | Implemented |
...
| RAI-ACCT-003 | Architecture documentation maintenance | Accountability | Architecture docs kept reasonably current in the task; local drift validation is available but is not an approval/deployment gate | `docs/architecture/*`, `scripts/validate-architecture.mjs` | No | — | Implemented |
...
| RAI-PRIV-003 | Privacy-safe telemetry | Privacy & Security | Analysis events record bounded operational fields and allowlisted Azure filter source/category/severity; blocked-key sanitisation excludes image bytes, Base64, identifiers, raw errors, prompts, clinical responses, transcripts, tokens and secrets | `lib/telemetry/server.ts`, `lib/telemetry/analysis-events.ts`, `app/api/analyze-wound/route.ts` | No | `tests/rai/rai-telemetry.test.ts` | Implemented |
...
| RAI-INCL-003 | AI output language consistency | Inclusiveness | New output is language-validated; existing HCP/History narratives use text-only translation with EN/MS caching while canonical and numeric values are validated unchanged and images are not resent | `lib/ai/language.ts`, `lib/ai/analysis/translation.ts`, `app/api/analyze-wound/translate/route.ts`, HCP analysis/history clients | Yes | `tests/unit/ai-language.test.ts`, `tests/unit/analysis-translation.test.ts`, `tests/rai/rai-controls.test.ts` | Implemented |
