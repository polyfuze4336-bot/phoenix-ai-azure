...
| UI-CLINICAL-NOTICE | Clinical AI legal/safety notice | UI | Next.js/React | `components/clinical-ai-notice.tsx`, Original HCP analysis and chat | Concise bilingual confidentiality, PDPA and decision-support reminders without a compliance claim | UI-I18N, UI-HCP | ACTIVE | Phoenix AI team |
...
| DEVOPS-GHA | GitHub Actions | DevOps | GitHub Actions + OIDC | `.github/workflows/*` | Direct-main rapid-prototype CI signal and automatic Development deployment through `github-phoenixai-deploy`; Demo/infra/DB operations remain manual | OPS-GHA-OIDC-RBAC | ACTIVE | Phoenix AI team |
...
| GOV-VALIDATE | Architecture drift script | DevOps | Node/TypeScript | `nextjs_space/scripts/validate-architecture.mjs` | Mandatory local drift detection before direct-main push; not server-enforced | — | ACTIVE | Phoenix AI team |
