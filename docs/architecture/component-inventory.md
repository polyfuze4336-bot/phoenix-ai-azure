...
| API-HCP-CHAT | HCP chat route | API | Next.js route handler | `app/api/hcp-chat/route.ts` | HCP clinical chat; narrative response follows validated EN/BM selection | AI-PROVIDER, PROMPT-HCP-CHAT | ACTIVE | Phoenix AI team |
| API-HCP-ANALYSIS | HCP wound analysis route | API | Next.js route handler | `app/api/analyze-wound/route.ts` | Multimodal wound/burn assessment (staged pipeline default; single-pass fallback) with one-or-many image input, consolidated total TBSA and validated EN/BM narrative output | AI-ANALYSIS-PIPELINE, AI-MODEL-SELECTOR, AI-PROVIDER, AI-VALIDATION, PROMPT-HCP-ANALYSIS | ACTIVE | Phoenix AI team |
...
| AI-STREAMING | AI streaming helpers | Lib | Web Streams/SSE | `lib/ai/streaming/*` | Stream responses, robustly process completed and trailing SSE frames, and share analysis completion parsing across original/v2 clients | — | ACTIVE | Phoenix AI team |
...
| AI-ANALYSIS-PIPELINE | Staged wound-analysis orchestrator | Lib | TypeScript | `lib/ai/analysis/pipeline.ts` | 4-stage analysis + deterministic safety calc + deterministic TBSA major/minor classification + EN/BM clinician-facing narrative selection | AI-PROVIDER, AI-ANALYSIS-SCHEMA, AI-MODEL-SELECTOR, CLINICAL-PARKLAND, CLINICAL-TBSA, PROMPT-ANALYSIS-STAGES | ACTIVE | Phoenix AI team |
