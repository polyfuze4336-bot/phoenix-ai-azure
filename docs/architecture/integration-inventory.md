...
| --- | --- | --- | --- | --- | --- | --- | --- |
| INT-BROWSER-APP | Browser (client) | Next.js server (`app/`) | HTTPS | Serve one Phoenix AI experience; submit the empty-by-default HCP demo login form for server verification; submit all chat/image requests with validated `en`/`ms`; translate existing structured HCP results without resending images; reject unsupported images and invalid language values explicitly | Demo login uses client session state; Entra uses a signed session cookie | User-entered demo login payload, persisted UI language, chat text, ephemeral JPEG/PNG/WebP/GIF base64 image payloads, existing structured result for text-only translation | ACTIVE |
