...
| INT-COMMUNITY-YOUTUBE | Community browser | YouTube privacy-enhanced embed (`www.youtube-nocookie.com`) | HTTPS iframe | Stream the optional unlisted burn first-aid education video only on its Community page | None | Validated 11-character YouTube video ID embedded in a normalized player URL; raw environment value and unrelated configuration are not exposed | OPTIONAL |
