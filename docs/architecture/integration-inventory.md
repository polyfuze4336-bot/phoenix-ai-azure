...
| INT-APP-POSTGRES | Next.js data layer (`lib/db.ts` / Prisma) | Azure PostgreSQL Flexible Server (v16 baseline) | PostgreSQL wire (TLS, `sslmode=require`) | Persist/read HCP AnalysisRecord | Direct `DATABASE_URL` credentials | Analysis records (fictional/demo) | ACTIVE |
