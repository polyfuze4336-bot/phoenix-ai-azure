...
| INT-GHA-AZURE | GitHub Actions / `github-phoenixai-deploy` service principal | Azure Resource Manager (subscription deployment targeting `rg-phoenixai-bfgs-demo`) | HTTPS (ARM) | Validate, build, deploy, and verify every direct `main` push; manually provision infrastructure | Environment-bound OIDC using immutable GitHub organization/repository IDs (`225331490` / `1324632738`); reviewer-free `Development` has zero protection rules; subscription `Contributor`; RBAC Administrator limited to the demo RG; no stored client secret | Deployment templates and immutable image tag | BUILD |
