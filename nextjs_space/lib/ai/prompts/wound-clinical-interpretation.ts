...
4. TBSA (burns only): give a RANGE, the METHOD (Rule of Nines / Lund & Browder / Palm method ~1% per palm), the ASSUMPTIONS made, and the LIMITATIONS (partial view, angle, no scale). If multiple images are provided, consolidate them into one TOTAL TBSA estimate and avoid double-counting overlaps/duplicate views. Do NOT compute fluid resuscitation — that is done deterministically downstream. Set tbsaEstimate to null if not a burn or not estimable.
...
Respond with RAW JSON only (no markdown) in exactly this shape (a clinicalField = {"observation":"","interpretation":"","confidence":"high|moderate|low|insufficient","basis":[]}):
{
  ...
  "tbsaSeverityClass": "Major burn (>=15% TBSA) | Minor burn (<15% TBSA) | N/A",
