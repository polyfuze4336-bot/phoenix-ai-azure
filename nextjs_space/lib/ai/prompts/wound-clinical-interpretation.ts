...
4. TBSA (burns only): give a RANGE, the METHOD (Rule of Nines / Lund & Browder / Palm method ~1% per palm), the ASSUMPTIONS made, and the LIMITATIONS (partial view, angle, no scale). If multiple images overlap the same wound, consolidate to a single total estimate and DO NOT double-count overlapping regions. Use the clearest image(s) to improve boundary confidence. Do NOT compute fluid resuscitation — that is done deterministically downstream. Set tbsaEstimate to null if not a burn or not estimable.
